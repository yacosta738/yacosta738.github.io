...
test("test i18n and rss", async ({ page }) => {
	...
	await expect(page.locator('h2:has-text("Otros proyectos")')).toHaveText(
		"Otros proyectos",
	);
	await expect(page.locator('h2:has-text("Últimos artículos")')).toHaveText(
		"Últimos artículos",
	);
	await expect(page.locator('h2:has-text("Contáctame")')).toHaveText(
		"Contáctame",
	);
	...
	await expect(
		page.locator('h2:has-text("Other Noteworthy Projects")'),
	).toHaveText("Other Noteworthy Projects");
	await expect(page.locator('h2:has-text("Last Articles")')).toHaveText(
		"Last Articles",
	);
	await expect(page.locator('h2:has-text("Contact me")')).toHaveText(
		"Contact me",
	);
});
