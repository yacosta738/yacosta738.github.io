...
test('test i18n and rss', async ({ page }) => {
	...
	await expect(await page.locator('h2:has-text("Dónde he trabajado")')).toHaveText('Dónde he trabajado')
	await expect(await page.locator('h2:has-text("Otros proyectos")')).toHaveText('Otros proyectos')
	await expect(await page.locator('h2:has-text("Últimos artículos")')).toHaveText('Últimos artículos')
	...
	await expect(await page.locator('h2:has-text("Where I\'ve Worked")')).toHaveText("Where I've Worked")
