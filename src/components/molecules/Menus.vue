...
const addEventToClassName = (className: string, func: Function, event = 'click'): void => {
...
const openSearchBox = (): void => {
	...
	const searchBox: HTMLInputElement = document.querySelector('#search-box') as HTMLInputElement
...
<template>
	<ul class="menu-list" :class="$generalStore.drawer ? 'block' : 'hidden'" data-cypress="menu">
		...
		<li>
			<a href="/files/yuniel_acosta_cv.pdf" target="_blank" class="resume-button">
				{{ t('resume') }}
			</a>
		</li>
	</ul>
