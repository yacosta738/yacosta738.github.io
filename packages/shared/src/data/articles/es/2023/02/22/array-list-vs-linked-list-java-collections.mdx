...
- Buscar en un **`LinkedList`** significa seguir los enlaces en **_`O(n)`_** tiempo para el peor caso, mientras que en un **`ArrayList`** la posición deseada se puede calcular matemáticamente utilizando _la dirección base y el desplazamiento_ y se puede acceder en **_`O(1)`_**.
